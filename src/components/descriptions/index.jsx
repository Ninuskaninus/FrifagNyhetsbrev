import React from "react";

const Descriptions = {
  Top: () => {
    return (
      <>
      <p>✅ Huk av for de nyhetsbrevene du vil ha</p>
      <p>💌 Skriv inn epostadressa di nederst på sida</p>
      <p>👇 Trykk «Meld meg på»</p>
      </>
    )
  },

  FriFagbevegelse: () => {
    return (
      <>
      <h5>FriFagbevegelse</h5>
      <p>Nyheter, reportasjer og debatt fra arbeidslivet og fagbevegelsen</p>
      </>
    )
  },

  Rorsla: () => {
    return (
      <>
      <h5>Rørsla</h5>
      <p>Få nye episoder av podkasten Rørsla rett i innboksen</p>
      </>
    )
  },

  Utenriks: () => {
    return (
      <>
      <h5>Utenriks</h5>
      <p>Nyheter og reportasjer fra internasjonalt arbeidsliv og fagbevegelse</p>
      </>
    )
  },

  Tillitsvalgte: () => {
    return (
      <>
        <h5>Nyhetsbrev for tillitsvalgte</h5>
        <p>Reportasjer, nyheter og artikler som er nyttige for deg som er tillitsvalgt i et LO-forbund </p>
      </>
    )
  },

  MagForFag: () => {
    return (
      <>
      <h5>Magasinet for fagorganiserte</h5>
      <p>Nyheter og reportasjer om ansatte i store bransjer i privat sektor, blant annet industri, bygg, transport og hotell og restaurant</p>
      </>
    )
  },


  HKNytt: () => {
    return (
      <>
      <h5>HK-Nytt</h5>
      <p>Nyheter og reportasjer om ansatte i handel, kontor, luftfart og reiseliv, finans, forlag, media, samferdsel og organisasjoner</p>
      </>
    )
  },

  Arbeidsmanden: () => {
    return (
      <>
      <h5>Arbeidsmanden</h5>
      <p>Nyheter og reportasjer om arbeidsfolk i bransjer som maskin, anlegg, vakt, renhold og bergverk</p>
      </>
    )
  },
    Nettverk: () => {
    return (
      <>
      <h5>Nettverk</h5>
      <p>Nyheter og reportasjer om folk som jobber i bransjer som elektro, energi, telekom, heis og IT</p>
      </>
    )
  },
    NNN: () => {
    return (
      <>
      <h5>NNN-arbeideren</h5>
      <p>Nyheter og reportasjer om ansatte i mat- og drikkevareindustrien</p>
      </>
    )
  },



  NTL: () => {
    return (
      <>
      <h5>NTL-magasinet</h5>
      <p>Nyheter og reportasjer om ansatte i staten og virksomheter med statlig tilknytning</p>
      </>
    )
  },



  Fontene: () => {
    return (
      <>
      <h5>Fontene</h5>
      <p>Nyheter og reportasjer om helse- og sosialarbeidere</p>
      </>
    )
  },

  NJF: () => {
    return (
      <>
      <h5>NJF-magasinet</h5>
      <p>Nyheter og reportasjer om ansatte i jernbane og busstransport</p>
      </>
    )
  },

  Posthornet: () => {
    return (
      <>
      <h5>Posthornet</h5>
      <p>Nyheter og reportasjer om ansatte i Posten, Bring og DNB</p>
      </>
    )
  },


  Iskolen: () => {
    return (
      <>
      <h5>I skolen</h5>
      <p>Nyheter og reportasjer om ansatte i skolesektoren</p>
      </>
    )
  },

  NFF: () => {
    return (
      <>
      <h5>NFF-magasinet</h5>
      <p>Nyheter og reportasjer om ansatte i fengsel og friomsorg</p>
      </>
    )
  },

  LoAktuelt: () => {
    return (
      <>
      <h5>Nyhetsbrev for tillitsvalgte</h5>
      <p>Reportasjer, nyheter og artikler for deg som er tillitsvalgt i et LO-forbund</p>
      </>
    )
  },

  AltOmLønnsoppgjøret: () => {
    return (
      <>
      <h5>Alt om lønnsoppgjøret</h5>
      <p>Alt du trenger å vite om lønna di</p>
      </>
    )
  },

  MagasinetStyrke: () => {
    return (
      <>
      <h5>Magasinet Styrke</h5>
      <p>Nyheter og reportasjer om ansatte i olje-, gass- og landindustri, ledere, ingeniører og teknikere</p>
      </>
    )
  },

  MaritimLogg: () => {
    return (
      <>
      <h5>Maritim logg</h5>
      <p>Nyheter og reportasjer om sjøfolk, havfiskere og ansatte i maritime yrker.</p>
      </>
    )
  },


}

export default Descriptions;
